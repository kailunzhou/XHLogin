...
class ForgotPasswordSetCtrl: CTBaseCtrl {
    private var mainView = ForgotPasswordSetView()
    public var mobileStr = ""
    public var smscodeStr = ""
    ...
    private func buildUI() {
        ...
        mainView = ForgotPasswordSetView(frame: view.bounds)
    ...
    private func resetPassword(_ password: String) {
        LoginConfig.share.resetPassword(mobileStr, smscodeStr, password) { [weak self] in
...
class ForgotPasswordSetView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        titleLabel.attributedText = NSAttributedString(string: "请设置新的登录密码", attributes: [.foregroundColor: UIColor(hexString: "353535"), .font: UIFont.systemFont(ofSize: 16)])
        ...
        showBtn.setImage(UIImage(named: "login_closeeye", ForgotPasswordSetView.self), for: .normal)
        showBtn.setImage(UIImage(named: "login_openeye", ForgotPasswordSetView.self), for: .selected)
    ...
    @objc private func clickMethod(_ sender: UIButton) {
        ...
        case nextBtn:
            guard let password = passwordTF.text, password.count > 0 else {
                MBProgressHUD.showMessage(nil, with: "请输入新密码", complete: nil)
                break
            }
            if !password.regularPassword() {
                MBProgressHUD.showMessage(nil, with: "请输入符合条件的新密码", complete: nil)
                break
            }
...
extension ForgotPasswordSetView: UITextFieldDelegate {
